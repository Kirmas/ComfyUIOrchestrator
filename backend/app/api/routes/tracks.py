...
from app.core.track_order import ordered_tracks, splice_after, unlink_track
...
async def create_track(payload: TrackCreate, db: AsyncSession = Depends(get_db)):
    """Ordering is a per-project linked list (core/track_order.py), so a new
    ...
    if payload.after_track_id is not None:
        ...
        # No anchor given and not head -> append at the current tail.
        ordered = await ordered_tracks(db, payload.project_id)
    ...
    db.add(track)
    await db.flush()
    await splice_after(db, payload.project_id, track, after, at_head=payload.place_at_head)
    await db.commit()
