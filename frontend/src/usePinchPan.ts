 ...
 * a container element. Used by ZoomableImage (single image viewer) and
 * CompareModal (its zoom mode) so the two behave identically -- the pinch math
...
export function usePinchPan(containerRef: RefObject<HTMLDivElement | null>, enabled = true) {
  ...
  const zoomToward = (nextZoomRaw: number, focalClientX: number, focalClientY: number) => {
    ...
    setView((v) => {
      ...
      if (z1 === MIN_ZOOM) return { zoom: 1, x: 0, y: 0 };
  ...
  const onPointerDown = (e: React.PointerEvent) => {
    ...
    if (pts.length >= 2) {
      ...
      setGrabbing(false);
    } else if (viewRef.current.zoom > MIN_ZOOM) {
